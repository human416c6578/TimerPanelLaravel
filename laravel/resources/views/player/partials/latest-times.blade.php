...
<table class="min-w-full text-left text-sm">
                    <thead class="speed-table-head sticky top-0 z-10">
                        <tr>
...
@foreach ($latestTimes as $record)
    <tr class="speed-row">
        ...
        <td class="px-4 py-2">
            <a class="speed-link" href="/maps/{{ $record->MapUUID }}">
                {{ $record->MapName }}
            </a>
        </td>
        <td class="px-4 py-2">{{ $record->CategoryName }}</td>
        <td class="px-4 py-2 font-mono text-amber-200">{{ formatTime($record->Time) }}</td>
        <td class="px-4 py-2">{{ $record->RecordDate }}</td>

        <td class="px-6 py-3 border-b border-slate-700 xl:text-sm text-xs xl:text-medium text-slate-400">
